...
--3. Realizar un bloque PL para visualizar los artículos de un número de compra leído de teclado. Obtener
...
DECLARE
  CURSOR C1 (NCOMP NUMBER) IS 
    SELECT NUM_COMPRA , CODARTI ,DENOMINACION ,CATEGORIA ,UNIDADES,PVP , (PVP* UNIDADES) AS IMPORTE,(STOCK+UNIDADES) AS STOCK_ACTUAL
    FROM DET_COMPRAS JOIN ARTICULOS USING (CODARTI) 
    WHERE NUM_COMPRA=NCOMP;
  --REGISTRO PARA PROCESAR DATOS DEL CURSOR 
  REG C1%ROWTYPE;
  N_COMP DET_COMPRAS.NUM_COMPRA%TYPE := &NUMERO_DE_COMPRA;
  NC COMPRAS.NUM_COMPRA%TYPE;
  TOTAL_UNIDADES NUMBER := 0; -- Declaración de TOTAL_UNIDADES
  TOTAL_IMPORTE NUMBER := 0; -- Declaración de TOTAL_IMPORTE
BEGIN
  --VER SI NUM_COMPRAS EXISTE
  SELECT NUM_COMPRA INTO NC FROM COMPRAS WHERE NUM_COMPRA = N_COMP;
  ...
  LOOP
    ...
    DBMS_OUTPUT.PUT_LINE(RPAD(TO_CHAR(REG.NUM_COMPRA),12,' ')||RPAD(TO_CHAR(REG.CODARTI),16,' ')||
            RPAD(TO_CHAR(REG.DENOMINACION),16,' ') ||   RPAD(TO_CHAR(REG.CATEGORIA),6,' ') || RPAD(TO_CHAR(REG.UNIDADES),6,' ')|| RPAD(TO_CHAR(REG.PVP),7,' ')|| RPAD(TO_CHAR(REG.IMPORTE),7,' ')|| RPAD(TO_CHAR(REG.STOCK_ACTUAL),7,' '));
  ...
  --IMPRIMIR TOTALES
  DBMS_OUTPUT.PUT_LINE(' ');
  DBMS_OUTPUT.PUT_LINE('TOTAL_UNIDADES: '||TOTAL_UNIDADES);
  DBMS_OUTPUT.PUT_LINE('TOTAL_IMPORTE: '||TOTAL_IMPORTE);

EXCEPTION 
  WHEN NO_DATA_FOUND THEN
    DBMS_OUTPUT.PUT_LINE('LA COMPRA NO EXISTE ' );
END;
