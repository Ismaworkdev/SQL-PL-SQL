...
--1. Crear un procedimiento almacenado al que le pasaremos un c digo de academia, de tal forma que nos mostrar  todos sus profesores y para cada profesor los datos de los opositores que se han preparado en esa academia.
--La salida tendr  el siguiente formato:
--NOMBRE DE LA ACADEMIA: xxxxxxxxxxxxxxxxxx   
---------------------------------------------------------------------------------------------------------------
----PROFESOR: xxxxxxxxxxxx  
---------------------------------------------------------------------------------------------------------------
--	NOMBRE ALUMNO          APELLIDOS ALUMNO         FECHA NACIMIENTO	xxxxxxxxxxxxxxx             xxxxxxxxxxxxxxx 	xxxxxxxxxxxxx	         	   
...
--xxxxxxxxxxxxxxx             xxxxxxxxxxxxxxx 	xxxxxxxxxxxxx	         
---------------------------------------------------------------------------------------------------------------
--N MERO TOTAL DE PROFESORES: XXXXX                      N MERO TOTAL DE ALUMNOS: XXXXX
---------------------------------------------------------------------------------------------------------------
 SET SERVEROUTP ON
SET VERIFY OFF

--COMO HACER QUE EL PROCEDURE COJA UN CODIGO DE ACADEMIA Crear un procedimiento almacenado al que le pasaremos un c digo de academia
CREATE OR REPLACE PROCEDURE PROC_ACADEMIA (P_COD_ACADEMIA ACADEMIAS.COD_ACADEMIA%TYPE) AS
--ACADEMIA 

CURSOR C1 IS
SELECT  * FROM ACADEMIAS
WHERE COD_ACADEMIA = P_COD_ACADEMIA;
VARIABLE_ACADEMIA C1%ROWTYPE;
--OPOSITOR ACADEMIA PROFESOR

CURSOR C2(VARIABLE_ACADEMIA ACADEMIAS.COD_ACADEMIA%TYPE) IS
SELECT * FROM OPOSITOR_ACADEMIA_PROFESOR
WHERE COD_ACADEMIA = VARIABLE_ACADEMIA;
VARIABLE_O_A_P C2%ROWTYPE;
--PROFESOR

CURSOR C3(VARIABLE_A_PO OPOSITOR_ACADEMIA_PROFESOR.COD_PROFESOR%TYPE)IS
SELECT * FROM PROFESORES
WHERE COD_PROFESOR = VARIABLE_A_PO;
VARIABLE_PROFESORES C3%ROWTYPE;
--OPOSITOR
CURSOR C4(VARIABLE_A_P_O OPOSITOR_ACADEMIA_PROFESOR.COD_OPOSITOR%TYPE) IS
SELECT * FROM OPOSITORES
WHERE COD_OPOSITOR = VARIABLE_A_P_O;
VARIABLE_OPOSITORES C4%ROWTYPE;

BEGIN

OPEN C1;
LOOP
  FETCH C1 INTO VARIABLE_ACADEMIA;
        EXIT WHEN C1%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE('--------------------------------------------------------------------------------------------------------------- ' );
        DBMS_OUTPUT.PUT_LINE('NOMBRE DE LA ACADEMIA: ' || VARIABLE_ACADEMIA.NOM_ACADEMIA);


          OPEN C2(VARIABLE_ACADEMIA.COD_ACADEMIA);
        LOOP
            FETCH C2 INTO VARIABLE_O_A_P;
            EXIT WHEN C2%NOTFOUND;


OPEN C3(VARIABLE_O_A_P.COD_PROFESOR);
LOOP
  FETCH C3 INTO VARIABLE_PROFESORES;
        EXIT WHEN C3%NOTFOUND;  
         DBMS_OUTPUT.PUT_LINE('PROFESORES: ' || VARIABLE_PROFESORES.NOM_PROFESOR);
    
        
          OPEN C4(VARIABLE_O_A_P.COD_OPOSITOR);
        LOOP
            FETCH C4 INTO VARIABLE_OPOSITORES;
...
DBMS_OUTPUT.PUT_LINE( VARIABLE_OPOSITORES.NOM_OPOSITOR||'           '||VARIABLE_OPOSITORES.APE_OPOSITOR||'             '||VARIABLE_OPOSITORES.FECHA_NAC);
...
CALL PROC_ACADEMIA(1);

--TRIGGER
---?	Realizar un trigger para controlar las operaciones de inserción, modificación y borrado en la tabla de OPOSITORES. 
--?	Si se realiza una operación de inserción se grabará en AUDITA_OPOSITORES, la fecha y la hora, “INSERCIÓN”.
--?	Si se realiza una operación de modificación sobre los campos nombre, apellidos, se grabará en AUDITA_OPOSITORES, la fecha y hora, “MODIFICACIÓN”, y además el valor anterior y nuevo de las columnas modificadas, sólo de las columnas modificadas.
--?	Si se realiza una operación de borrado se grabará en AUDITA_OPOSITORES, la fecha y la hora, “BORRADO” y además todos los datos del usuario que se ha borrado.

--La tabla AUDITA_OPOSITORES, no existe, debes crearla utilizado el lenguaje de definición de datos, con un campo que tenga suficiente tamaño, para almacenar los datos que se piden. 
--Debes incluir el código para la creación de la tabla, junto con la respuesta a la pregunta.
--CREAR PRIMERO LA TABLA 
DROP TABLE AUDITA_OPOSITORES CASCADE CONSTRAINTS;
CREATE TABLE AUDITA_OPOSITORES (
    fechayhora  DATE NOT NULL,
    TIPO_OPERACION   VARCHAR2(100 CHAR) NOT NULL,
    COD_OPERACION   NUMBER NOT NULL,
    MODIFICACION_VALORES   VARCHAR2(100 CHAR)
); 

--Trigger
CREATE OR REPLACE TRIGGER AUDITA_OPOSITORES_
AFTER INSERT OR DELETE OR UPDATE  ON OPOSITORES
FOR EACH ROW
DECLARE
...
BEGIN
  IF INSERTING THEN
   INSERT INTO AUDITA_OPOSITORES VALUES(Sysdate ,'INSERTING',( SELECT NVL(MAX(COD_OPERACION),0)+1 FROM AUDITA_OPOSITORES) , :NEW.NOM_OPOSITOR);
  
  ELSIF DELETING THEN
     INSERT INTO AUDITA_OPOSITORES VALUES(Sysdate ,'DELETING',( SELECT NVL(MAX(COD_OPERACION),0)+1 FROM AUDITA_OPOSITORES) , :OLD.NOM_OPOSITOR);

     ELSIF UPDATING THEN
        INSERT INTO AUDITA_OPOSITORES VALUES(Sysdate ,'UPDATING',( SELECT NVL(MAX(COD_OPERACION),0)+1 FROM AUDITA_OPOSITORES) , (:OLD.NOM_OPOSITOR|| '   '|| :NEW.NOM_OPOSITOR));
  END IF;


END AUDITA_OPOSITORES_;



DELETE OPOSITORES 
WHERE COD_OPOSITOR = 4342;


UPDATE OPOSITORES 
SET NOM_OPOSITOR = 'LOS MEGA QUE PAPI '
WHERE COD_OPOSITOR = 4442;


INSERT INTO OPOSITORES VALUES(4442,'LOL','PPP',TO_DATE('30/12/75', 'DD/MM/RR'));
INSERT INTO OPOSITORES VALUES(4342,'LORRRL','PPRRP',TO_DATE('30/12/00', 'DD/MM/RR'));
