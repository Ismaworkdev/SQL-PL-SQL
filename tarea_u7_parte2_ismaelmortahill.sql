...
DECLARE
    ID clientes.idclientes%TYPE;
    fecha pedido.fecha%type;
    helado pedido.id_paquete_helado%type;
    trabajador pedido.idtrabajadores%type;
    cli pedido.idclientes%type;
    precio pedido.precio%type;
    v_pedido pedido%ROWTYPE; 
    CURSOR c_pedidos IS
    SELECT p.id_paquete_helado, p.IDtrabajadores, p.IDClientes, p.fecha, p.precio
        FROM Pedido p
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Información del pedido del cliente con DNI ' || ID || ':');
    DBMS_OUTPUT.PUT_LINE('-----------------------------------------------------------------------------');
    DBMS_OUTPUT.PUT_LINE('ID Paquete Helado       ID Transportista     ID Cliente       Fecha     Precio         ');
    DBMS_OUTPUT.PUT_LINE('------------------ ------------------- ------------------- -------------- ------------------');

    -- Abrir el cursor de pedidos
    OPEN c_pedidos;
    LOOP
        ...
        DBMS_OUTPUT.PUT_LINE(v_pedido.id_paquete_helado || '                        ' || v_pedido.IDtrabajadores || '                  ' || v_pedido.IDClientes || '               ' || v_pedido.fecha || '           ' || v_pedido.precio);
    END LOOP;
    CLOSE c_pedidos;

EXCEPTION 
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('No se encontró información de pedidos para el cliente con DNI ' || ID);
    WHEN OTHERS THEN
